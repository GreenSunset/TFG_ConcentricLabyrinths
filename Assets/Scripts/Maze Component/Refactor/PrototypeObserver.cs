...
public class PrototypeObserver : MonoBehaviour
{
    static public PrototypeObserver main { get; private set; }
    public PrototypeNavigator target;
    ...
    void Update()
    {
        ...
        if (target.nextPointIndex < 0) {
            normal = maze.GetRotation(target.currentPointIndex) * Vector3.up;
        } else {
            normal = Vector3.Lerp(maze.GetRotation(target.currentPointIndex) * Vector3.up, maze.GetRotation(target.nextPointIndex) * Vector3.up, target.current2nextRatio);
        }
