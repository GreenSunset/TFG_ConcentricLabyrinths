...
public class MazeVisualizer : MonoBehaviour
{
    ...
    void GenerateMesh(int level = -1) {
        ...
        levels[level].transform.localPosition = Vector3.zero;
        foreach (GameObject light in lights) {
            light.transform.parent = levels[level].transform;
            light.transform.localPosition = light.transform.position;
        }
    ...
    void OnDrawGizmos() {
        ...
        for (int i = 0; i < nPoints; i++) {
            ...
            float ratio = Mathf.Abs(idxLevel - currentLevel) / 1.1f;
